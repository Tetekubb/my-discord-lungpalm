...
class MusicBot(commands.Bot):
    def __init__(self):
        ...
        intents.message_content = True # ต้องเปิดใน Discord Developer Portal ด้วยนะพี่
    ...
    async def setup_hook(self):
        ...
        print(f"✅ Tete Music System พร้อมทำงาน!")

bot = MusicBot()

def get_embed(song, user, guild):
...
async def play_next(guild, channel, user):
    if guild.id in bot.queue and bot.queue[guild.id]:
        ...
        search = f"เพลงคล้าย {last['title']}" if last else "เพลงฮิตไทย"
...
async def start_playing(guild, channel, user, song):
    ...
    try:
        ...
        await channel.send(embed=get_embed(song, user, guild), view=TeteMusicView(bot, guild.id))
